/**
 * CORE GAME ENGINE
 * Soldier Frontline: Operation Survival (Weather Warning Update)
 * - Lava: 3s Warning (Blinking) before rising
 * - Thunderstorm: Slower strike (Increased warning time)
 * - Drop Rate: High (Shield 35%, Heal 20%)
 * - No I-Frames
 */
...
function spawnSpecialEvents() {
    ...
    if (frames > 0 && frames % 900 === 0) { // Every 15 seconds
        ...
        if (rand < 0.35) {
            type = 'SHIELD'; // 35% Chance
        } else if (rand < 0.55) { // 0.35 to 0.55 = 20% Chance
            type = 'HEAL';   
        } else if (rand < 0.70) {
...
function handleWeather() {
    ...
    if (weather === 'ACID_RAIN') {
        ...
        if (frames % 60 === 0) {
            ...
            // [MOD] Increased warning time to 50 frames (approx 0.8s) for better reaction time
            lightningStrikes.push({ x: strikeX, timer: 50, state: 'WARN' }); 
        }
        ...
        let warningDuration = 180; // [MOD] 3 Seconds Warning

        let lavaH = 110;
        let currentLavaY = floorY - lavaH;

        if (activeTime < warningDuration) {
            // [MOD] WARNING PHASE: Blinking Orange
            ...
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 20px Arial';
            ctx.fillText("!!! LAVA RISING !!!", canvas.width/2 - 100, currentLavaY - 20);
        } else {
            // [MOD] ACTIVE PHASE: Deadly
            ctx.fillStyle = 'rgba(255, 69, 0, 0.85)';
            ctx.fillRect(0, currentLavaY, canvas.width, lavaH);

            if(frames % 20 === 0) {
                 createParticles(Math.random()*canvas.width, currentLavaY, 1, '#ffff00');
            }
